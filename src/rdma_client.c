...
void init_send_wr(enum SENDOP op, struct ibv_mr *mr) {

    client_send_sge.addr = (uint64_t)mr->addr;
    client_send_sge.length = (uint32_t)mr->length;
    client_send_sge.lkey = mr->lkey;
    /* now we link to the send work request */
    bzero(&client_send_wr, sizeof(client_send_wr));
...
static int client_xchange_metadata_with_server() {
	...
	// send client_metadata
	init_send_wr(SEND, client_metadata_mr);
    /* Now we post it */
...
static int client_remote_memory_ops() {
	...
	init_send_wr(WRITE, client_src_mr);
    /* Now we post it */
    ret = ibv_post_send(client_qp, &client_send_wr, &bad_client_send_wr);
    if (ret) {
	...
	/* Step 2: is to copy the remote buffer into the local buffer. We will
     * check if it is right */
	init_send_wr(READ, client_dst_mr);
    /* Now we post it */
    ret = ibv_post_send(client_qp, &client_send_wr, &bad_client_send_wr);
    if (ret) {
        rdma_error(
