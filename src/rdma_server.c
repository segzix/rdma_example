...
void init_send_wr(enum SENDOP op, struct ibv_mr *mr) {
    ...
    server_send_sge.addr = (uint64_t)mr->addr;
    server_send_sge.length = (uint32_t)mr->length;
    server_send_sge.lkey = mr->lkey;
    /* now we link to the send work request */
    ...
    case WRITE:
        ...
        server_send_wr.wr.rdma.rkey = server_metadata_attr.stag.remote_stag;
        server_send_wr.wr.rdma.remote_addr = server_metadata_attr.address;
        break;
    case READ:
